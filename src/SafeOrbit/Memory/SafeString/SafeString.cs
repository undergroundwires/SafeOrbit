...
{
    //write tests for public void Insert(int position, ISafeBytes character, Encoding encoding = Encoding.Utf16LittleEndian)
    public class SafeString : ISafeString
    {
        ...
        public SafeString() : this(LibraryManagement.Factory.Get<ITextService>(),
        ...
        {
            this.Insert(Length, character, encoding);
        }
        /// <exception cref="ArgumentNullException">
        ...
        public void Append(ISafeString safeString)
        {
            ...
            {
                this.Append(safeString.GetAsSafeBytes(i).DeepClone());
            }
        }
        ...
        public void AppendLine()
        {
            ...
            this.Append(safeBytes, InnerEncoding);
        }
        ...
        /// <exception cref="ObjectDisposedException">Throws if the <see cref="SafeString"/> instance is disposed.</exception>
        /// <exception cref="InvalidOperationException">Throws if the <see cref="SafeString"/> instance is empty.</exception>
        /// <exception cref="ArgumentOutOfRangeException">Throws if <paramref name="index"/> is less than zero, higher than/equals to the length.</exception>
        ...
        public void Remove(int startIndex, int count = 1)
        {
            ...
            {
                throw new ArgumentOutOfRangeException($"Total of {nameof(count)}({count}) and {nameof(startIndex)}({startIndex} is higher than the length({Length})");
            }
        ...
        public bool Equals(ISafeString other)
        {
            ...
            for (var i = 0; i < this.Length; i++)
            {
                if (!this.GetAsSafeBytes(i).Equals(other.GetAsSafeBytes(i)))
                {
                    return false;
                }
            }
        ...
        public ISafeString DeepClone()
        {
            ...
            for (var i = 0; i < this.Length; i++)
            {
                result.Append(this.GetAsSafeBytes(i).DeepClone(), InnerEncoding);
            }
            return result;
        }
        /// <exception cref="ObjectDisposedException">Throws if the <see cref="SafeString"/> instance is disposed</exception>
        public ISafeString ShallowClone()
        {
            ...
            return this.MemberwiseClone() as ISafeString;
        }
        public void Dispose()
        {
            this.Clear();
        ...
        {
            byte[] byteBuffer = safeBytes.ToByteArray();
            try
            {
